...
export class TokenRoomManager {
  ...
  private async _storeRoomReference(
    ...
    try {
      ...
      if (!roomId || !roomName) {
        console.warn(
          "🔍 _storeRoomReference: Invalid room data, skipping save"
        );
        return;
      }
      ...
      if (!roomReference.id || !roomReference.name) {
        console.warn(
          "🔍 _storeRoomReference: Missing required fields, skipping save"
        );
        return;
      }
    ...
    const handler = roomMessagesNode.on(
      async (messageData: any, messageId: string) => {
        ...
        if (messageData && messageId && messageData.timestamp) {
          ...
          if (isNewMessage) {
            console.log("🚀 Processing NEW token room message:", messageId);
            ...
            console.log(
              "🚀 Ignoring OLD token room message:",
              messageId,
              "age:",
              messageAge,
              "ms"
          ...
          console.log(
            "🚀 Processing token room message without timestamp:",
            messageId
          );
          await this._processIncomingMessage(
            messageData,
  ...
  private async _loadMessagesFromLocalStorage(roomId: string): Promise<any[]> {
    try {
      ...
      if (storedMessages) {
        ...
        console.log(
          "📱 TokenRoom: Loaded messages from localStorage:",
          messages.length
        );
        return messages;
      }
      ...
      console.log(
        "📱 TokenRoom: No messages in localStorage for room:",
        roomId
      );
      return [];
    } catch (error) {
      console.warn("⚠️ TokenRoom: Error loading from localStorage:", error);
      return [];
    }
  }
  ...
  private async _saveMessageToLocalStorage(
    ...
    try {
      ...
      console.warn("⚠️ TokenRoom: Error saving to localStorage:", error);
    }
  }
  ...
  public async createTokenRoom(
    ...
    try {
      ...
      if (
        ...
        console.warn(
          "🔍 createTokenRoom: Missing required room data, cannot create room"
        );
        return {
          success: false,
  ...
  public async getTokenRoomData(roomId: string): Promise<TokenRoomData | null> {
    ...
    if (!roomId) {
      console.log("🔍 getTokenRoomData: No roomId provided");
      return null;
    }
    ...
    try {
      ...
      console.log(
        "🔍 getTokenRoomData: Active rooms:",
        Array.from(this.activeTokenRooms.keys())
      );

      const roomNode = this.core.db.gun.get(roomPath);
      console.log("🔍 getTokenRoomData: Created room node:", roomNode);

      return new Promise((resolve) => {
        // Try to get room data with timeout
        const timeout = setTimeout(() => {
          console.log(
            "🔍 getTokenRoomData: Timeout reached, trying fallback methods"
          );
          // Try fallback: check if room exists in active rooms
          if (this.activeTokenRooms.has(roomId)) {
            console.log("🔍 getTokenRoomData: Room found in active rooms");
            const token = this.activeTokenRooms.get(roomId);
            ...
            console.log(
              "🔍 getTokenRoomData: Room not found in active rooms either"
            );
            resolve(null);
          }
        }, 5000); // 5 second timeout

        console.log("🔍 getTokenRoomData: Starting GunDB lookup...");
        roomNode.get("data").on((roomData: any) => {
          clearTimeout(timeout);
          console.log("🔍 getTokenRoomData: Received data for room:", {
            roomId,
            roomPath,
          ...
          if (roomData && roomData.id === roomId) {
            console.log(
              "🔍 getTokenRoomData: Room found successfully:",
              roomData
            );
            resolve(roomData as TokenRoomData);
          } else {
            console.log("🔍 getTokenRoomData: Room not found or ID mismatch");
            console.log("🔍 getTokenRoomData: Room data ID:", roomData?.id);
            console.log("🔍 getTokenRoomData: Expected ID:", roomId);
            console.log(
              "🔍 getTokenRoomData: ID match:",
              roomData?.id === roomId
            );
            ...
            if (this.activeTokenRooms.has(roomId)) {
              console.log(
                "🔍 getTokenRoomData: Room found in active rooms as fallback"
              );
              const token = this.activeTokenRooms.get(roomId);
              const fallbackRoomData: TokenRoomData = {
                id: roomId,
      ...
      console.error("🔍 getTokenRoomData: Error getting room data:", error);
      return null;
    }
  }
